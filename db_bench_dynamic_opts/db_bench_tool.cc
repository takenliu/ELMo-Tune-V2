...
DEFINE_string(
  dynamic_options_file, "", "File containing dynamic options to be set");
...
// -------------------------
// Added for tuned benchmark
// -------------------------

// Key access distribution
DEFINE_string(key_access_distribution, "random", "Key access distribution: random, zipfian, exp, two_term_exp");

DEFINE_double(zipfian_theta, 0.99, "Theta value for zipfian distribution");
DEFINE_double(normal_mean, 0.0, "Mean value for normal distribution");
DEFINE_double(normal_stddev, 0.0, "Standard deviation for normal distribution");

// Throughput distribution
DEFINE_string(qps_distribution, "uniform", "QPS distribution: uniform, sine, random-spikes");

DEFINE_double(sine_period, 0.0, "Period of sine wave in seconds");
DEFINE_double(sine_amplitude, 0, "Amplitude of sine wave (in bytes/sec)");
DEFINE_double(sine_offset, 0, "Offset of sine wave from base (in bytes/sec)");

// Query ratio
DEFINE_double(get_ratio, 0, "Read ratio in the workload");
DEFINE_double(put_ratio, 1, "Write ratio in the workload");
DEFINE_double(seek_ratio, 0, "Get ratio in the workload");

// Time-based workload (using json)
DEFINE_string(workload_file, "", "File containing the workload in json format");

// Client Properties
DEFINE_int32(num_clients, 1, "Number of clients to simulate");

// Value size distribution
DEFINE_string(value_size_dist, "fixed", "Value size distribution: fixed, noisy, pareto");

DEFINE_double(value_size_noise, 0.0, "Noise for noisy value size distribution");
DEFINE_double(value_size_mean, 0.0, "Mean value size for normal distribution");
DEFINE_double(value_size_stddev, 0.0, "Standard deviation for normal distribution");
 ...
 public:
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "tunable") {
        method = &Benchmark::TunableBench;
 ...
 private:
  ...
  void OpenDynamicOptionsFile() {
    ...
    if (mmap_dynamic_file_desc_ == -1) {
      fprintf(stderr, "Failed to open dynamic options file %s\n",
              FLAGS_dynamic_options_file.c_str());
      ErrorExit();
    }

    mmap_dynamic_file_ = mmap(nullptr, 1024, PROT_WRITE, MAP_SHARED, 
                              mmap_dynamic_file_desc_, 0);
    if (mmap_dynamic_file_ == MAP_FAILED) {
      fprintf(stderr, "Failed to mmap dynamic options file %s\n",
              FLAGS_dynamic_options_file.c_str());
      ErrorExit();
    }

    mmap_dynamic_file_addr_ = static_cast<char *>(mmap_dynamic_file_);
  }
  ...
  void CloseDynamicOptionsFile() {
    ...
    if (mmap_dynamic_file_desc_ != 0) {
      close(mmap_dynamic_file_desc_);
      mmap_dynamic_file_desc_ = 0;
    }
  }

  void LoadAndApplyDynamicOptions() {
    // Use db_ and SetDBOptions to change options while online
    ...
    if (mmap_dynamic_file_addr_[0] == 1) {
      ...
      int mddf_max_background_jobs = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 13);
      int mddf_max_background_compactions = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 17);
      int mddf_max_subcompactions = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 21);
      int mddf_stats_dump_period_sec = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 25);
      int mddf_compaction_readahead_size = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 29);
      int mddf_writable_file_max_buffer_size = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 33);
      int mddf_bytes_per_sync = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 37);
      int mddf_wal_bytes_per_sync = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 41);
      int mddf_delayed_write_rate = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 45);
      int mddf_avoid_flush_during_shutdown = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 49);

      int mddf_write_buffer_size = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 53);
      int mddf_compression = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 57);
      int mddf_level0_file_num_compaction_trigger = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 61);
      int mddf_max_bytes_for_level_base = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 65);
      int mddf_disable_auto_compactions = *reinterpret_cast<int*>(mmap_dynamic_file_addr_ + 69);
      ...
      fprintf(stderr, "max_total_wal_size: %d\n", mddf_max_total_wal_size);
      fprintf(stderr, "max_background_jobs: %d\n", mddf_max_background_jobs);
      fprintf(stderr, "memtable_max_range_deletions: %d\n", mddf_memtable_max_range_deletions);
    }

    mmap_dynamic_file_addr_[0] = 0;

    // Temporary solution to close the file after reading
    // CloseDynamicOptionsFile();
  }
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    while ((num_per_key_gen != 0) && !duration.Done(entries_per_batch_)) {
      ...
      for (int64_t j = 0; j < entries_per_batch_; j++) {
        ...
        // Load dynamic options if specific
        if (FLAGS_dynamic_options_file != "" and thread->tid == 0) {
          LoadAndApplyDynamicOptions();
        }
...
// optimization, so skip the check of the max level.
  ...
  // New class to generate the key based on a zipfian distribution
  class GenerateZipfianDistribution {
   public: 
    // Avoid uninitialized warning-as-error in some compilers
    int64_t zipfian_rand_max_ = 0;
    int64_t zipfian_num_ = 0;
    double zipfian_theta_ = 0.0;
    double zipfian_zetan_ = 0.0;
    double zipfian_alfa_ = 0.0;
    std::vector<double> zipfian_eta_;
    std::vector<double> zipfian_uz_;
    std::vector<int64_t> zipfian_key_;
    std::vector<int64_t> zipfian_key_offset_;

    // Initiate the zipfian distribution
    Status InitiateZipfianDistribution(int64_t total_keys, double zipfian_theta) {
      zipfian_theta_ = zipfian_theta;
      zipfian_num_ = total_keys;
      zipfian_alfa_ = 1.0 / (1.0 - zipfian_theta_);
      zipfian_zetan_ = Zeta(total_keys, zipfian_theta);
      zipfian_eta_.resize(total_keys);
      zipfian_uz_.resize(total_keys);
      zipfian_key_.resize(total_keys);
      zipfian_key_offset_.resize(total_keys);
      for (int64_t i = 0; i < total_keys; i++) {
        zipfian_eta_[i] = (1.0 - std::pow((i + 1), -zipfian_theta_)) /
                          (1.0 - std::pow(total_keys + 1, -zipfian_theta_));
        zipfian_uz_[i] = zipfian_eta_[i] * zipfian_zetan_;
      }
      zipfian_rand_max_ = static_cast<int64_t>(zipfian_uz_[total_keys - 1]);
      return Status::OK();
    }

    int64_t GenerateZipfianKey(int64_t ini_rand) { 
      int64_t key = 0;
      int64_t rand_num = ini_rand % zipfian_rand_max_;
      int64_t low = 0;
      int64_t high = zipfian_num_ - 1;
      while (low <= high) {
        int64_t mid = (low + high) / 2;
        if (zipfian_uz_[mid] >= rand_num) {
          key = mid;
          high = mid - 1;
        } else {
          low = mid + 1;
  ...
  // void static JsonThreadedBenchmark(const json& benchmark) {
    ...
    std::string name = "tunable";
    if (benchmark.contains("benchmark")) {
      name = benchmark["benchmark"].get<std::string>();
    }
    ...
    if (benchmark.contains("duration")) {
      FLAGS_duration = benchmark["duration"].get<int>();
    }
    if (benchmark.contains("key_access_dist")) {
      FLAGS_key_access_distribution = benchmark["key_access_dist"].get<std::string>();
    }
    if (benchmark.contains("zipfian_theta")) {
      FLAGS_zipfian_theta = benchmark["zipfian_theta"].get<double>();
    }
    if (benchmark.contains("qps_distribution")) {
      FLAGS_qps_distribution = benchmark["qps_distribution"].get<std::string>();
    }
    if (benchmark.contains("sine_period")) {
      FLAGS_sine_period = benchmark["sine_period"].get<int>();
    }
    if (benchmark.contains("sine_amplitude")) {
      FLAGS_sine_amplitude = benchmark["sine_amplitude"].get<int>();
    }
    if (benchmark.contains("sine_offset")) {
      FLAGS_sine_offset = benchmark["sine_offset"].get<int>();
    }
    if (benchmark.contains("get_ratio")) {
      FLAGS_get_ratio = benchmark["get_ratio"].get<double>();
    }
    if (benchmark.contains("put_ratio")) {
      FLAGS_put_ratio = benchmark["put_ratio"].get<double>();
    }
    if (benchmark.contains("seek_ratio")) {
      FLAGS_seek_ratio = benchmark["seek_ratio"].get<double>();
    }
    if (benchmark.contains("num_clients")) {
      FLAGS_num_clients = benchmark["num_clients"].get<int>();
    }
    if (benchmark.contains("value_size_dist")) {
      FLAGS_value_size_dist = benchmark["value_size_dist"].get<std::string>();
    }
    if (benchmark.contains("value_size_mean")) {
      FLAGS_value_size_mean = benchmark["value_size_mean"].get<int>();
    }
    if (benchmark.contains("value_size_stddev")) {
      FLAGS_value_size_stddev = benchmark["value_size_stddev"].get<int>();
    }
    if (benchmark.contains("value_size_noise")) {
      FLAGS_value_size_noise = benchmark["value_size_noise"].get<int>();
    }
  ...
  void TunableBench(ThreadState* thread) {
    int64_t gets = 0;
    int64_t puts = 0;
    int64_t get_found = 0;
    int64_t seek = 0;
    ...
    bool use_prefix_modeling = false;
    bool use_random_modeling = false;
    bool use_zipfian_modeling = false;
    GenerateTwoTermExpKeys gen_exp;
    GenerateZipfianDistribution gen_zipf;
    std::vector<double> ratio{FLAGS_get_ratio, FLAGS_put_ratio, FLAGS_seek_ratio};
    char value_buffer[default_value_max];
    QueryDecider query;
    RandomGenerator gen;
    Status s;
    ...
    if (FLAGS_key_access_distribution == "random") {
      use_random_modeling = true;
    } else if (FLAGS_key_access_distribution == "exp") {
      // We reuse the two term exp distribution for key access distribution,
      // only using key_dist_a and key_dist_b.
      use_prefix_modeling = true;
      gen_exp.InitiateExpDistribution(
          FLAGS_num, FLAGS_keyrange_dist_a, FLAGS_keyrange_dist_b, 0, 0);
    } else if (FLAGS_key_access_distribution == "two_term_exp") {
      use_prefix_modeling = true;
      gen_exp.InitiateExpDistribution(
          FLAGS_num, FLAGS_keyrange_dist_a, FLAGS_keyrange_dist_b,
          FLAGS_keyrange_dist_c, FLAGS_keyrange_dist_d);
    } else if (FLAGS_key_access_distribution == "zipfian") {
      use_zipfian_modeling = true;
      gen_zipf.InitiateZipfianDistribution(FLAGS_num, FLAGS_zipfian_theta);
    } else if (FLAGS_key_access_distribution == "normal") {
      // Yet to implement. throw error for now.
      fprintf(stderr, "Normal distribution not implemented yet.\n");
      exit(1);
      // The following need to be set
      // - FLAGS_normal_mean
      // - FLAGS_normal_stddev
    } else {
      fprintf(stderr, "Unknown key access distribution: %s\n",
              FLAGS_key_access_distribution.c_str());
      exit(1);
    }
    ...
    if (FLAGS_qps_distribution == "sine") {
      FLAGS_sine_mix_rate = true;
      FLAGS_sine_a = FLAGS_sine_amplitude * (FLAGS_key_size + FLAGS_value_size);
      FLAGS_sine_b = FLAGS_sine_period/3.14159265;
      FLAGS_sine_d = FLAGS_sine_offset * (FLAGS_key_size + FLAGS_value_size);
    } else if (FLAGS_qps_distribution == "uniform") {
      FLAGS_sine_mix_rate = false;
    } else if (FLAGS_qps_distribution == "random-spikes") {
      // Yet to implement. throw error for now.
      fprintf(stderr, "Random spikes distribution not implemented yet.\n");
      exit(1);
    } else {
      fprintf(stderr, "Unknown QPS distribution: %s\n",
              FLAGS_qps_distribution.c_str());
      exit(1);
    }

    if (FLAGS_value_size_dist == "fixed") {
      // This is the default setting. No need to do anything.
    } else if (FLAGS_value_size_dist == "noisy") {
      FLAGS_sine_mix_rate_noise = FLAGS_value_size_noise;
    } else if (FLAGS_value_size_dist == "pareto") {
      // The following must be set. If set, it is present from mixgraph.
      // - FLAGS_value_theta
      // - FLAGS_value_k
      // - FLAGS_value_sigma
    } else {
      fprintf(stderr, "Unknown value size distribution: %s\n",
              FLAGS_value_size_dist.c_str());
      exit(1);
    }
    ...
    while (!duration.Done(1)) {
      ...
      if (use_random_modeling) {
        ...
        key_rand =
            gen_exp.DistGetKeyID(ini_rand, FLAGS_key_dist_a, FLAGS_key_dist_b);
      } else if (use_zipfian_modeling) {
        key_rand = gen_zipf.GenerateZipfianKey(ini_rand);
      } else {
        key_seed = PowerCdfInversion(u, FLAGS_key_dist_a, FLAGS_key_dist_b);
        Random64 rand(key_seed);
        key_rand = static_cast<int64_t>(rand.Next()) % FLAGS_num;
